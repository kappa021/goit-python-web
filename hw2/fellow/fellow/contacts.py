...
class Email(Field):
    ...
    def value(self, value):
        try:
            valid_email = re.match(r"^.+@(\w+\.){0,2}[a-z]{2,6}$", value, re.IGNORECASE)
            self._value = valid_email
        except ValueError:
...
class Record:
    ...
    def add_email(self, email_arg):
        self.email.append(Email(email_arg))
    ...
    def __str__(self):
        ...
        if isinstance(self.birthday,Birthday) and isinstance(self.email, Email):
            birthday_str = self.birthday.value.strftime("%d/%m/%y")
            res = f"{self.name} {', '.join([str(phone) for phone in self.phones])} {birthday_str} {', '.join([str(email) for email in self.email])}"
        else:
